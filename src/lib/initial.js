...
export const INITIAL_COMPOSITION = `<style>
  ...
  /* Theme via four CSS vars so the palette-swap plugin (and any
   * future theming surface) can recolor without rewriting the
   * composition. Fallbacks match the original look so compositions
   * without a palette set are unchanged. */
  body { margin: 0; width: 100vw; height: 100vh;
         background: var(--cw-bg, #0c0a09);
         color: var(--cw-fg, #fafaf9);
  ...
  .stat-label { font-size: clamp(12px, 2vmin, 22px);
                color: var(--cw-mute, #a8a29e); }
  .outro b { color: var(--cw-accent, #fed7aa); }
  .pill { display: inline-flex; padding: 0.6vmin 1.6vmin; border-radius: 999px;
          background: color-mix(in srgb, var(--cw-fg, #fafaf9) 8%, transparent);
          border: 1px solid color-mix(in srgb, var(--cw-fg, #fafaf9) 18%, transparent);
          color: var(--cw-accent, #fed7aa);
